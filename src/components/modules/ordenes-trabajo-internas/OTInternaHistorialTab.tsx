...
export default function OTInternaHistorialTab({ otId }: { otId: number }) {
  const [data, setData] = useState<HistorialRecord[]>([]);
  const [loading, setLoading] = useState(false);
  const { ocultas, setOcultas } = useColumnasOcultas("ot-interna-historial-cols-v1");
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const res = await fetch(`/api/ordenes-trabajo-internas/${otId}/historial`);
        if (res.ok && alive) {
  ...
  const { columnas: columnsResizable, components: tableComponents, resetAnchos, TableDragWrapper } =
    useColumnasRedimensionables<HistorialRecord>(columns, "ot-interna-hist-cols-widths-v1");
