...
export async function POST(req: NextRequest) {
  ...
  const [otAdj, reqAdj, compraG, compraF, compraP, compraMulti, evalI] = await Promise.all([
    ...
    prisma.compraAdjunto.findFirst({ where: { r2_key: key }, select: { id: true } }),
    prisma.evaluacionTecnica.findFirst({ where: { informe_key: key }, select: { id: true } }),
  ]);
  if (otAdj || reqAdj || compraG || compraF || compraP || compraMulti || evalI) {
