...
// Nota: la promoción de recursos_status ("Recursos solicitados") solo aplica a OTs externas.
export async function POST(req: NextRequest, ctx: Ctx) {
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      return { enviados: borradores.length, total: items.length };
    });

    return NextResponse.json({ data: result });
  } catch (error) {
    if (error instanceof Error && error.message === "NOT_FOUND") {
      return NextResponse.json({ error: "Requerimiento no encontrado en esta OT interna." }, { status: 404 });
    }
    ...
    console.error("POST enviar grupo (OT interna) error:", error);
