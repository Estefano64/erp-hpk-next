...
// body: { po_id: number, items: [{material_id, cantidad, observacion?}], usuario: string }
export async function POST(req: NextRequest) {
  try {
    const body = await req.json();
    const { po_id, items, usuario } = body;
      ...
      for (const item of items) {
        ...
        const mov = await tx.movimientoInventario.create({
          data: {
            ...
            cantidad: cant,
            documento_referencia: compra.numero_po,
            observacion: item.observacion || `Recepción de OC ${compra.numero_po}`,
            usuario,
          },
        });
