...
export default function AceptacionesPage() {
  ...
  useEffect(() => {
    fetch("/api/me").then((r) => r.ok ? r.json() : null).then((d) => { if (d?.user) setRol(d.user.rol); }).catch(() => {});
  }, []);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/aceptaciones?${params}`);
      if (res.ok) setData(await res.json());
    } catch {
      message.error("Error al cargar aceptaciones");
    } finally {
      setLoading(false);
    }
  ...
  const ocColumns: ColumnsType<OCPendiente> = [
    numeracionColumn<OCPendiente>({ current: pageOC, pageSize }),
    {
      ...
      render: (_, o) => (
        <Popover content={popoverOC(o)} placement="rightTop" trigger="hover" mouseEnterDelay={0.3}>
          <div style={{ cursor: "help", display: "flex", alignItems: "center", gap: 4 }}>
            <InfoCircleOutlined style={{ color: brand.cyan, fontSize: 11 }} />
            <Tag color={brand.navy}>{o.numero_po}</Tag>
          </div>
        </Popover>
      ),
    },
    ...
    },
    {
      ...
      render: (_, o) => (
        <Space size={2}>
          <Tooltip title="Ver OC completa">
            <Button type="text" size="small" icon={<EyeOutlined />} onClick={() => router.push(`/compras`)} />
          </Tooltip>
          {isAdmin && (
            <Tooltip title="Aceptar OC (pasa a En Proceso)">
              <Popconfirm
                title={`¿Aceptar OC ${o.numero_po}?`}
              ...
              >
                <Button type="primary" size="small" icon={<CheckOutlined />}>Aceptar</Button>
              </Popconfirm>
            </Tooltip>
          )}
        </Space>
  ...
  const rqColumns: ColumnsType<ReqPendiente> = [
    numeracionColumn<ReqPendiente>({ current: pageRQ, pageSize }),
    {
      ...
      render: (_, r) => (
        <Popover content={popoverRQ(r)} placement="rightTop" trigger="hover" mouseEnterDelay={0.3}>
          <div style={{ cursor: "help", display: "flex", alignItems: "center", gap: 4 }}>
            <InfoCircleOutlined style={{ color: brand.cyan, fontSize: 11 }} />
    ...
    {
      key: "acciones", title: "Acciones", width: 170, fixed: "right", align: "center",
      render: (_, r) => (
        <Space size={2}>
          {isAdmin && (
          ...
          {isAdmin && (
            <Tooltip title="Desaprobar">
              <Button type="text" size="small" danger icon={<CloseOutlined />} onClick={() => desaprobarReq(r)} />
            </Tooltip>
          )}
        </Space>
      ),
    ...
    <div>
      ...
      {!isAdmin && (
        <Alert
          ...
          title="Modo lectura"
          description="Solo administradores pueden aceptar OCs y aprobar requerimientos. Podés ver y filtrar lo pendiente."
        />
      )}
              ...
              <>
                ...
                {(filterTipo === "ALL" || filterTipo === "OC") && (
                  <Card
                    ...
                    extra={
                      <Space>
                        {isAdmin && selOcs.length > 0 && (
                          <Popconfirm
                            title={`¿Aceptar ${selOcs.length} OC(s) seleccionada(s)?`}
                            onConfirm={bulkAceptarOC}
                            okText="Aceptar" cancelText="Cancelar"
                          >
                            <Button type="primary" icon={<CheckOutlined />} size="small">
                              Aceptar seleccionadas ({selOcs.length})
                            </Button>
                          </Popconfirm>
                        )}
